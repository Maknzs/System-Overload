...
export default function Login({ onLogin, goBack }) {
  ...
  function mapLoginError(error) {
    ...
    if (status === 400)
      return "Please enter a valid email and an 8+ character password.";
    ...
    try {
      ...
      try {
        ...
        if (
          status === 401 &&
          (code === "BAD_CREDENTIALS" ||
            /invalid credentials/i.test(le.message || ""))
        ) {
      ...
      if (ENABLE_BETTER_AUTH) {
        const { token: baToken, user: baUser } = await api(
  ...
  return (
    <>
      <Seo
        title="Log In"
        description="Access your System Overload account to continue coordinating squads, tracking match history, and unlocking new bot strategies."
        canonicalPath="/login"
        keywords={["system overload login", "multiplayer account access"]}
        noindex
      />
      <div className="page">
